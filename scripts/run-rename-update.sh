...
#   2. ALERT!!!! `../service` BE CAREFUL AND MODIFY THIS. The directory is tricky here. I can't know where (a) you're running the command or (b) where you've stored these docker files. 
...
find ../service -mindepth 1 -maxdepth 1 -type d -exec ln -sbf ../.env {}/.env \;
